...
def create_one(tenant: schemas.TenantCreate, db: Session = Depends(get_db), current_user: schemas.Account = Depends(get_current_account)):
    db_tenant = get_tenant_by_name(db, name=tenant.name)
    if db_tenant:
        raise HTTPException(status_code=400, detail="Name already registered")

    return create_tenant(db, tenant=tenant)
